...
export const importGrowwHoldings = async (files) => {
  ...
  const pricedHoldings = await applyCommonMarketPrices(allHoldings);
  const importedInstrumentTypes = [...new Set(
    pricedHoldings
      .map((holding) => String(holding?.instrumentType || "").trim())
      .filter(Boolean)
  )];

  if (importedInstrumentTypes.length > 0) {
    await Holding.deleteMany({
      broker: "groww",
      instrumentType: { $in: importedInstrumentTypes }
    });
  }
  ...
  return {
    ...
    byFile,
    replacedInstrumentTypes: importedInstrumentTypes,
    sample: pricedHoldings.slice(0, 5)
  };
};
